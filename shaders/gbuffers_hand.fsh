#version 330 compatibility
/* Solid-geometry gbuffer fragment stage (terrain/entities/hand/basic). */
#include "/lib/lumen_uniforms.glsl"
#include "/lib/lumen_gbuffer.glsl"

/* RENDERTARGETS: 0,1,2,3 */
layout(location = 0) out vec4 outColor;
layout(location = 1) out vec4 outNormalAO;
layout(location = 2) out vec4 outLightSpec;
layout(location = 3) out vec4 outEmission;

in vec2  v_texcoord;
in vec2  v_lmcoord;
in vec4  v_tint;
in vec3  v_viewNormal;
in vec3  v_viewPos;
in float v_matId;

void main() {
    GbufferData g = buildGbuffer(v_texcoord, v_lmcoord, v_tint, normalize(v_viewNormal), v_matId);
    if (g.color.a < 0.1) discard;
    outColor     = g.color;
    outNormalAO  = g.normalAO;
    outLightSpec = g.lightSpec;
    outEmission  = g.emissionSSS;
}
